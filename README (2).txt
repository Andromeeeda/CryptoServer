...
Хеширование пароля добавить в пакет pkg
В структуре User должен хранить хеш-пароль пользователя. 
Сначала нужно через библиотеку bcrypt сделать функцию для создания хеш-пароля.
Далее сделать функцию для проверки хеш-пароля с паролем введенным пользователем.  
и срок токена(не закончился ли срок токена) так же проверить существует ли пользователь 

Сделал первую рабочую версию проекта 23.04.26
Далее нужно:
1.Внедрить БД вместо хранения внутри программы в переменных(в оперативной памяти)
2.Обернуть в докер
Исправить ошибки:
1.Сделать(переделать) на более продуманную логику проверки аутентификации чтобы не дублировать код middleware 
2.Синхронизировать автообновление в консоли с выводом хенделеров shedule 
3. Исправить ошибку что в cryptoservice не находит по методу getCoinInfo BTC(bitcoin)