...
Логика реализации аутентификации:
Клиент в своем запросе в хедере делает аутентификацию вида: `Authorization: Bearer <token>`.
Моя задача реализовать функцию которая будет проверять корректность введенного токена. Проверять:
Наличие токена, корректность формата хедера, корректность токена(подпись не подделан ли токен)
и срок токена(не закончился ли срок токена) так же проверить существует ли пользователь 